...
int main()
{
    ...
    ret=mysql_real_connect(ret,"localhost","root","1","mysql_203",0,NULL,0);   //连接数据库
	if(ret==0)
	{
		printf("数据库链接失败！\n");
	}
					...
					else//当读取到客户端输入的内容时
					{
						...
						if(msg.type==1)//设置注册为一类信息
						{
							...
							{								
								...
								mysql_query(ret,buf1);  //执行sql语句，插入姓名，密码
							...
							{																					
								...
								for(int j=0;j<row;j++)//遍历每一行
								{
									...
									if((strcmp(myrow[0],msg.name) == 0) && (strcmp(myrow[1],msg.passwd) == 0))//将行信息中的第一行的第一个数据与输入的名字比较...看是否相同
									{
										flag = 1;
										break;//当找到相同内容时，停止遍历
									}
						...
						else if(msg.type==2)//设置销户为二类信息
						{
                            ...
                            for(int j=0;j<row;j++)//遍历每一行
                            {
                                flag=0;
                                myrow=mysql_fetch_row(str);//获取行的内容
                                if((strcmp(myrow[0],msg.name) == 0) && (strcmp(myrow[1],msg.passwd) == 0))//如果之前注册的信息和现在输入的信息相同，说明找到了要销户的账户
                                {
                                    flag = 1;
